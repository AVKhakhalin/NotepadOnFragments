package ru.geekbrains.lessions2345.notepadonfragments_2.model;

public class Constants {
    public static final int LIST_NAMES_SIZE = 23;
    public static final int LIST_DATES_SIZE = 17;
    public static final String KEY_CARD_SOURCE_IMPLEMENT = "CardSourceImplement";
    public static final String KEY_INDEX = "Index";
    public static final String KEY_CREATED_NEW_NOTE = "CreatedNewNote";
    public static final String KEY_DATA_SETTINGS = "Settings";
    public static final String KEY_GOOGLE_AUTHORISE = "GoogleAuthorise";
    public static final int TEST_DATA_INT = 0;
    public static final int FILE_DATA_INT = 1;
    public static final int FIREBASE_DATA_INT = 2;
    public static final int DATABASE_DATA_INT = 3;

    // Размер буфера карточек заметок (для работы с массивами дат)
    public static final int DELTA_CHANGE_INT_ARRAYS = 10;
    // По-умолчанию, короткое название пустой записи
    public static final String NAME_EMPTY_NOTE_ADD = "СОЗДАТЬ ЗАМЕТКУ";
    public static final String NAME_EMPTY_NOTE = "НОВ.ЗАМ.";
    public static final String DESCRIPTION_EMPTY_NOTE = "НОВАЯ ЗАМЕТКА";
}
